...
namespace SpatialHashing
{
                                    ...
                                    unsigned int blocksNum)
    {
        ...
        for (unsigned int g = 0; g < GROUP_PER_THREAD; g++)
        {
            ...
            int groupHomeCellEnd = groupStart + homeCellCount;
            for (int i = groupStart; i < groupHomeCellEnd; i++)
            {
                // 如果cell为EMPTY_CELL，则说明已经处理完了
                ...
                for (int j = i + 1; j < groupEnd; j++)
                {
                    ...
                    else
                    {
                        ...
                        calculateGridIndex(balls[ball2Index].position, cellSize, homeIndex2);
